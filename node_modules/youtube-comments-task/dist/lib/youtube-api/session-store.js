...
var extractCommentsToken = function extractCommentsToken(html) {
  var oldMatch = /'COMMENTS_TOKEN'\s*\n*:\s*\n*"([^"]+)"/i.exec(html);
  if (oldMatch && oldMatch[1]) {
    return Either.Right(decodeURIComponent(oldMatch[1]));
  }

  var newMatch = /"continuation":"([^"]+)".{0,500}"comment-item-section"/.exec(html);
  if (newMatch && newMatch[1]) {
    return Either.Right(newMatch[1]);
  }
